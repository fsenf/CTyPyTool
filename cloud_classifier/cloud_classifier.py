...
class cloud_classifier(cloud_trainer, data_handler):
    ...
    def create_training_filelist(self, verbose=True):
        satFile_pattern = fh.get_filename_pattern(
            self.sat_file_structure, self.timestamp_length
        )
        labFile_pattern = fh.get_filename_pattern(
            self.label_file_structure, self.timestamp_length
        )
        self.training_sets = fh.generate_filelist_from_folder(
    ...
    def create_training_set(self, verbose=True):
        ...
        fh.create_subfolders(path=filename)
    ...
    def extract_input_filelist(self, verbose=True):
        ...
        self.input_files = fh.generate_filelist_from_folder(
            folder=self.input_source_folder,
    ...
    def save_labels(self, labels, indices, sat_file, probas=None, verbose=True):
        name = fh.get_label_name(
            sat_file,
            self.sat_file_structure,
            self.label_file_structure,
